        ...
        if tieBreakInfo:
            if datePrints:
                for dp in datePrints:
                    ...
                    yield  "|".join(["kDatePrint_vDocId",fhrVer,dp]), 1
                ...
                yield  "|".join(["kDocId_vTieBreakInfo",fhrVer,docId]), 1
                ...
                yield  "|".join(["unlinkable",fhrVer,docId]), 1
                return
        else:
            #if there is no tieBreakInfo, the packet is bad.
            return

    def combiner(self, keyIn, valIter):
        self.increment_counter("COMBINER", "key in")
        yield keyIn, sum(valIter)


    def reducer(self, keyIn, valIter):
        self.increment_counter("REDUCER", "key in")
        total = sum(valIter)
        if total>10000:
            self.increment_counter("REDUCER", "key out")
            yield keyIn, str(total)
